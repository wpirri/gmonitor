...
int CWTimer::Ok(unsigned int id)
{
  ...
  if(i == m_vTimer.size()) return -1; /* no se encontr� */
...
/* Borra un o varios timer por transacci�n */
         ...
         const void* msg, unsigned long msg_len, bool repeat)
{
  ...
  /* tomo la hora actual para usar como referencia esto puede traer problemas
     de retrasos por la diferencia de tiempo entre la petici�n del servicio
...
int CWTimer::Check()
{
  ...
  /* si el vector est� vac�o salgo directamente */
  ...
  /* devuelvo el indice si encontr�, sino devuelvo -1 */
